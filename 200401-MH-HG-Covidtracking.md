...
```python
params = pd.DataFrame(index={'DC','MD','VA','NJ'}, columns=['fullname'])
params.loc[:,'fullname'] = pd.Series({ 'DC': 'District of Columbia', 'MD': 'Maryland', 'VA': 'Virginia'})
params.loc[:,'labYOff'] = pd.Series({ 'DC': +10, 'MD': +15, 'VA': +10, 'NJ':-15})
params.loc[:,'labXOff'] = pd.Series({ 'DC': 0, 'MD': +5, 'VA': +5, 'NJ': 0})
params.loc[:,'lw'] = pd.Series({ 'DC': 2, 'MD': 2, 'VA': 2, 'NJ': 0.8})
params.loc[:,'xoff'] = pd.Series({ 'DC': 0, 'MD': 0, 'VA': 0, 'NJ': -1})
```
...
todayx = 0 #26
def plot_state_death(state='DC', ax=ax, is_inset=False):
    global todayx
    desIx = ctDf.state == state
    ys = ctDf.loc[desIx,'death']
    dtV = pd.to_datetime(ctDf.loc[desIx,'date'], format='%Y%m%d')
    print(f'Latest data for {state}: {dtV.iloc[0]}')
    xs = (dtV - dtV.iloc[-1]) 
    xs = r_[[x.days for x in xs]] + params.loc[state, 'xoff'] #- todayx
...
#ylim = ax.get_ylim()
ax.set_ylabel('Deaths', fontsize=13)
ax.set_xlabel('Days', fontsize=13)

# guide lines
#xs = r_[3,xlim[1]+2] NEED TO FIX 
xs = r_[1,10]
dtL = [2,3,4]
for (iD,dt) in enumerate(dtL):
    ys = 2**(xs/dt)
    y2 = ys*10/(iD+1)
    ax.plot(xs, y2, '--', lw=0.5, color='0.6')
    ax.annotate('%d days to double'%dt, xy=(9,y2[1]*1.2), xycoords='data', fontsize=8, color='0.6')
...
axins.set_facecolor('#EAEAF2')
for st in ['DC', 'MD', 'VA']:
    plot_state_death(st, ax=axins, is_inset=True)
fixups(ax=axins)
...
fig.tight_layout()
fig.savefig('./fig-output/dt-%s.png'%datestr, dpi=300, bbox_inches='tight', bbox_extra_artists=[axins], pad_inches=0.5)
            #bbox_inches=r_[0,0,10,15])#, 
```
